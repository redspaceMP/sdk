...
A curated reference for the public surface of each package. For the full
declarations, see the `dist/index.d.ts` file in each package, or the
`src/index.ts` sources in this repository.

---

## `@redspacem/server-types`

The typed `mp.*` host contract plus pure helpers. No runtime dependencies.
...
```ts
clamp(value: number, min: number, max: number): number;      // inclusive [min, max]
distance(a: MpVector3, b: MpVector3): number;                // euclidean distance
...
Dependency-free, type-safe RPC. The transport is injectable and abstract.
...
class RpcServer {
  ...
  handle(raw: string): Promise<string>; // returns raw JSON response
}
```
...
A tiny DI container with lazy singletons.
...
class Container {
  ...
  resolve<T = unknown>(token: string): T;   // throws if unbound
...
Client-side script API (in-game, RED4ext-style). Depends on `server-types`
and re-exports `MpVector3`.
...
lerp(a: number, b: number, t: number): number; // linear interpolation, t clamped to [0, 1]
```
...
Incoming messages must be `{ event: string, payload: unknown }` envelopes;
...
Development-only helpers for unit tests.
...
Next: [Contributing](/contributing) · [Roadmap](/roadmap)
