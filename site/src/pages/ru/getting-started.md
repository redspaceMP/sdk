...
Это руководство проведёт вас через установку пакетов RedSpace SDK и написание
первого гейммод-ресурса.

## Требования

- **Bun 1.3+** (SDK собирается и тестируется с Bun). Установка — на
  [https://bun.sh](https://bun.sh). Репозиторий **не** использует npm/yarn.
- **Ядро сервера** RedSpace для запуска ресурса. Ядро — отдельный продукт с
...
Объект `mp` **не** создаётся вами — серверное ядро RedSpace внедряет
...
Дальше: [Архитектура](/ru/architecture) · [Справочник API](/ru/api)
